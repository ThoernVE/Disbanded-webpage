guild = {
    name: "",
    faction: "",
    region: "",
    realm: "",
    profile_url: "",
    raid_rankings: {
      nerubarPalace: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      blackrockDepths: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      awakenedAmirdrassilTheDreamsHope: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      awakenedAberrusTheShadowedCrucible: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      awakenedVaultOfTheIncarnates: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      amirdrassilTheDreamsHope: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      aberrusTheShadowedCrucible: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      vaultOfTheIncarnates: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      fatedSepulcherOfTheFirstOnes: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      fatedSanctumOfDomination: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      fatedCastleNathria: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      sepulcherOfTheFirstOnes: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      sanctumOfDomination: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      castleNathria: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      nyalothaTheWakingCity: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      theEternalPalace: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      crucibleOfStorms: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      battleOfDazaralor: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      uldir: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      antorusTheBurningThrone: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      tombOfSargeras: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      theNighthold: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      trialOfValor: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      },
      theEmeraldNightmare: {
        normal: {
          world: 0,
          region: 0,
          realm: 0
        },
        heroic: {
          world: 0,
          region: 0,
          realm: 0
        },
        mythic: {
          world: 0,
          region: 0,
          realm: 0
        }
      }
    },
    raid_progression: {
      nerubarPalace: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      blackrockDepths: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      awakenedAmirdrassilTheDreamsHope: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      awakenedAberrusTheshadowedCrucible: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      awakenedVaultOfTheIncarnates: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      amirdrassilTheDreamsHope: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      aberrusTheShadowedCrucible: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      vaultOfTheIncarnates: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      fatedSepulcherOfTheFirstOnes: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      fatedSanctumOfDomination: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      fatedCastleNathria: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      sepulcherOfTheFirstOnes: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      sanctumOfDomination: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      castleNathria: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      nyalothaTheWakingCity: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      theEternalPalace: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      crucibleOfStorms: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      battleOfDazaralor: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      uldir: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      antorusTheBurningThrone: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      tombOfSargeras: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      theNighthold: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      trialOfValor: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      },
      theEmeraldNightmare: {
        summary: "",
        expansion_id: 0,
        total_bosses: 0,
        normal_bosses_killed: 0,
        heroic_bosses_killed: 0,
        mythic_bosses_killed: 0
      }
    }
  }

function Guild(name, faction, region, realm, profile_url, raid_rankings, raid_progression,){
    this.name = name;
    this.faction = faction;
    this.region = region;
    this.realm = realm;
    this.profile_url = profile_url;
    this.raid_rankings = raid_rankings;
    this.raid_progression = raid_progression;
}

function GetGuildStats(region, realm, name){
    return fetch(`https://raider.io/api/v1/guilds/profile?region=${region}&realm=${realm}&name=${name}&fields=raid_rankings%2Craid_progression`)
    .then(response => response.json())
    .then(guildData => {return new Guild(
        guildData.name,
        guildData.faction,
        guildData.region,
        guildData.realm,
        guildData.profile_url,
        guildData.raid_rankings,
        guildData.raid_progression
    )
})
.catch(error => console.error("error fetching guild data:", error));
}

function PrintGuildData(region, realm, name){
    GetGuildStats(region, realm, name).then(newGuildData => {
        document.getElementById("raid_clear_one").innerText = newGuildData.raid_progression['blackrock-depths'].summary;
        document.getElementById("raid_rank_one").innerText = newGuildData.raid_rankings['blackrock-depths'].heroic.world + " World, " + 
        newGuildData.raid_rankings['blackrock-depths'].heroic.region + " Region, " + 
        newGuildData.raid_rankings['blackrock-depths'].heroic.realm + " Realm.";
        document.getElementById("raid_clear_two").innerText = newGuildData.raid_progression['blackrock-depths'].summary;
        document.getElementById("raid_rank_two").innerText = newGuildData.raid_rankings['nerubar-palace'].heroic.world + " World, " + 
        newGuildData.raid_rankings['nerubar-palace'].heroic.region + " Region, " + 
        newGuildData.raid_rankings['nerubar-palace'].heroic.realm + " Realm.";
      })

}

PrintGuildData("eu", "stormscale", "disbanded");